...
object StreamingLinearRegressionExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // $example on$
    val trainingData = ssc.textFileStream("./data/").map(LabeledPoint.parse).cache()
    val testData = ssc.textFileStream("./data/").map(LabeledPoint.parse)

    trainingData.count().print();
